 ...
 * @param {T | ((observer: ScrollObserver) => T)} value - 参数值
 * @param {ScrollObserver} scroller - 滚动观察者
 * @return {T} - 解析后的值
 */
...
/**
 * 注册并获取滚动容器
 * @param {TargetsParam} target - 目标元素
 * @return {ScrollContainer} - 滚动容器实例
 */
 ...
 * @param {Number} size - 容器大小
 * @param {Number} [under] - 下边界偏移
 * @param {Number} [over] - 上边界偏移
 * @return {Number} - 像素值
 */
