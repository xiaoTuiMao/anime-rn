...
class Transforms {
  /**
   * @param {DOMTarget|DOMProxy} $el - DOM元素或代理对象
   */
  constructor($el) {
    this.$el = $el;
  ...
  /**
   * 向上遍历父元素
   * @param {TraverseParentsCallback} cb - 回调函数
   */
  traverseUp(cb) {
 ...
 * @param {T | ((draggable: Draggable) => T)} value - 参数值
 * @param {Draggable} draggable - 可拖拽对象
 * @return {T} - 解析后的值
 */
...
export class Draggable {
  /**
   * 可拖拽构造函数
   * @param {TargetsParam} target - 拖拽目标
   * @param {DraggableParams} [parameters] - 拖拽参数
   */
   ...
   * 设置X坐标
   * @param {Number}  x - X坐标值
   * @param {Boolean} [muteUpdateCallback] - 是否静音更新回调
   * @return {this} - 返回当前实例
   */
